import { ValidateSchema, YMESchema } from '../schemas';

const rp = require('request-promise');

export default class YME {
  constructor(
    url,
    profile,
    site,
  ...
  getRaffleToken = async raffleId => {
    ...
    const response = await this.rp({
      ...
      headers: {
        ...
        pragma: 'no-cache',
        referrer: `https://ymeuniverse.typeform.com/to/${raffleId}`,
        referrerPolicy: 'no-referrer-when-downgrade'
      },
      uri: `https://ymeuniverse.typeform.com/app/form/result/token/${raffleId}/default`,
      form: {}
    });
  ...
  submitRaffleEntry = async (token, landedAt) => {
    ...
    this.raffleDetails.renderData.form.fields.forEach(row => {
      ...
      } else if (row.title.includes('size')) {
        formObj['4'] = {
          field: { id: row.id, type: row.type },
          text: this.size.id,
          type: 'text'
    ...
    const response = await this.rp({
      method: 'POST',
      json: true,
      uri: `https://ymeuniverse.typeform.com/app/form/submit/${this.raffleDetails.typeformCode}`,
      body: payload
    });
    return response;
  };
  ...
  makeEntry = async () => {
    ValidateSchema(YMESchema, { ...this.profile });
