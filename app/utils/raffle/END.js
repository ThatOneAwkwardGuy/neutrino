...
export default class END {
  ...
  loginToAccount = async () => {
    return new Promise(async (resolve, reject) => {
      ...
      win.webContents.on('dom-ready', () => {
        win.webContents.executeJavaScript('window.document.title', false, result => {
          if (result !== '') {
  ...
  addNewPayment = async (customer, token) => {
    const braintreeResponse = await this.rp({
      method: 'POST',
      uri: 'https://api.braintreegateway.com/merchants/s3qf7btpbbghkdyp/client_api/v1/payment_methods/credit_cards',
      body: {
        _meta: {
        ...
        authorizationFingerprint: `4ad22037f10b3c92039d653dc3c4971cce02949322a64c0837a39f8ac0279763|created_at=${new Date()
          .toISOString()
          .slice(0, -1)}000000+0000&merchant_id=s3qf7btpbbghkdyp&public_key=6t9dpqt857v6mpbz`
      }
    });
  ...
  makeEntry = async () => {
    ...
    let addressId;
    let paymentId;
    const addNewAddressResponse = await this.addNewAddress(customer, loginResponse);
